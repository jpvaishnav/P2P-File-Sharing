...
#specify the location where ip.txt file is present in your phone
file=open('Storage\0\emulated\qpython\ip.txt','r')
...
class Client(object):
    ...
    def download(self, num, title, peer_host, peer_port, peer_ip):
        try:
            ...
            #*** here if by some means we inculcate the IP information beforehead
            #it will be handful
            #this ip address is wlan ip address of the peer socket jisko ham file bhejenge
            #isko dusre laptop k command line se nikalke message k through pass karne ki try karo
